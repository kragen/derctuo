...
- topics pages
- PDF generation
- PDF failed on mismatched tag in lithium-fuel; also stm32duino (not
  well-formed) and sentential-database (mismatched tag) and
  hierarchical-state-space-learning (mismatched tag) (fixed with
  tidylib)
...
class Bundle:
    def __init__(self, dirname):
        ...
        self.output_dir = 'derctuo-' + self.get_version()
        self.cached_titles = {}
    ...
    def get_version(self):
        v = self.relation('version')['derctuo']
        return 'prerelease' if not v else v[0]

    def get_title(self):
        t = self.relation('bundle')['derctuo']
        return 'Derctuo' if not t else t[0]
    ...
    def generate_archive(self):
        subprocess.check_call('cd %s; tar czf derctuo-%s.tar.gz %s' % (
...
class Note:
    ...
    def author(self):
        return (self.bundle.relation('note-by')[self.notename]
                or self.bundle.relation('by')['derctuo']
                or ['Anonymous'])[0]
